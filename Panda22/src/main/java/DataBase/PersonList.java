...
/**
 * <h1>PersonList</h1> The PersonList class from DataBase Package implements to
 * create a list that contains amount of Person Objects, and read Person data
 * from the list in the class
 * <p>
 *
 * A PersonList contains an amount of Person in a list
 *
...
public class PersonList extends AbstractTableModel {
	...
	public void readCSV(String fileName) throws FileNotFoundException {
		...
		while (scanner.hasNext()) {
			...
			Person person = new Person(spilt[0], spilt[1], spilt[2], spilt[3], spilt[4], spilt[6], spilt[7], spilt[8]);
			list.add(person);
		}
		scanner.close();
	}
